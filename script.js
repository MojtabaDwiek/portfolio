...
function wrapTextSpans(selector){
  const el = document.querySelector(selector);
  if (!el) return;
  const text = el.textContent || "";
  let html = "";
  text.split("").forEach(function(dets){
    html += `<span>${dets}</span>`;
  });
  el.innerHTML = html;
}
...
function buildWorldGrid(){
  ...
  const europe = new Set([
    361,362,415,416,417,469,470,471,472,523,524,525,527,582,580,579,578,632,634,636,631,311,312,251,519,520,686,687,689,688,690,691,692,637,638,583,528,473,418,474,475,365,421,476,477,531,530,529,584,585,586,639,640,641,693,694,695,696,795,796,850,851,797,742,743,744,745,746,747,748,749,750,751,805,804,803,802,801,799,798,800,854,856,857,911
  ]);
  ...
  for (let i = 1; i <= total; i += 1) {
    ...
    cell.className = "cell";
    let className = "";
    if (northAmerica.has(i)) className = "dot-na";
    else if (southAmerica.has(i)) className = "dot-sa";
    else if (europe.has(i)) className = "dot-eu";
    else if (africa.has(i)) className = "dot-af";
    else if (asia.has(i)) className = "dot-as";
    else if (australia.has(i)) className = "dot-au";
    if (className) {
      cell.classList.add("dot", className);
      const duration = durations[className];
      cell.style.animationDelay = `${-Math.random() * duration}ms`;
    }
    grid.appendChild(cell);
  }
}

window.buildWorldGrid = buildWorldGrid;
if (document.readyState === "loading"){
  document.addEventListener("DOMContentLoaded", buildWorldGrid);
} else {
  buildWorldGrid();
}
